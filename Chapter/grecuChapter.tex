...
getting prominent in spaceborne radar applications with the work of \cite{iguchi_meneghini_1994}, \cite{meneghini1983}, 
and \cite{marzoug1991}. It should be noted that parameter $\beta$ has to be constant with range for the solution to be applicable.
...
is discussed in \cite{iguchi_meneghini_1994} and \cite{marzoug1991}.  Several ad-hoc techniques were developed to 
...
\begin{figure}\label{fig:retrievals}
    ...
    \caption{(Top) Differential PIA estimates from the ML model, the official DPR version 7 algorithm, the dual and single frequency (Ku)
    SRT method associated with the observations in Fig. \ref{fig:dataEx}.  A range of Hitschfeld-Bordan PIA derived using $N_w$ two times larger and smaller 
    than the reference value of 0.08cm$^4$ is also
    shown.(Bottom). The surface precipitation estimates from the Histchfeld-Bordan approach associated with the PIAs shown in the top panel 
    (labeled Prior) and the optimal surface precipitation estimate (labeled Final).}
\end{figure}
